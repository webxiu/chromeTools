...
/** 保存 */
const getDataInfo = () => JSON.parse(localStorage.getItem(Save_Info) || "{}");
const setDataInfo = (userInfo) =>
  localStorage.setItem(Save_Info, JSON.stringify(userInfo));
const removeDataInfo = () => localStorage.removeItem(Save_Info);
...
class CreateEle {
  ...
  init() {
    ...
    this.boxDom.innerHTML = `
        ...
        <form>
            <div class="form-item">
                <label for="element">元素选择器: </label> 
                <input type="text"  name="element" id="element" value="" placeholder="请输入" />
            </div> 
            <div class="form-item">
            ...
            <div class="form-item">
                <label for="result">比较值: </label>
                <input type="text"  name="result" id="result" value=""  placeholder="请输入值" />
            </div> 
